...
const SettingTimeline = () => {
	...
	return (
		<PageLayout>
			<Typography variant='h1'>타임라인 기본값 설정</Typography>
			<Typography variant='body2'>
				{
					'타임라인 기본값을 사용하여 타임라인의 모든 트윈에 동일한 설정을 적용합니다.\ntl.current = gsap.timeline({ defaults: { opacity: 0, ease: `back` } })와 같이 tl.current에만 해당하는 타임라인 설정을 만듭니다.\n\n여기서 알아야 할 것은 GSAP DOCS에서는 gsap.defaults 속성은 기본적인 애니메이션 설정에만 사용하고,\n메모리 관리와 같은 성능을 다루는 부분은 gsap.config()를 사용하기를 권장 한다는 점 입니다.'
				}
			</Typography>
			<Box
				ref={tweenRef}
