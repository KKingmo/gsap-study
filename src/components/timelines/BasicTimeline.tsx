...
useEffect(() => {
  const ctx = gsap.context(() => {
    ...
    tl.current.from('.sun', { duration: 1, opacity: 0, x: 50, y: 50 });
    tl.current.from('.gress', {
      duration: 1,
      opacity: 0,
      y: 100,
      stagger: { each: 0.2, from: 'center' },
    });
    tl.current.from('.bird', { duration: 1, opacity: 0, y: 100 });
    tl.current.from('.music', { duration: 1, opacity: 0, x: 100, y: 100 });
    tl.current.from('.eye-left', {
      duration: 0.1,
      x: 4,
      y: 3,
      rotate: 60,
...
const BasicTimeline = () => {
	...
	return (
		<PageLayout>
			<Typography variant='h1'>기초 타임라인</Typography>
			<Box ref={tweenRef}>
				<svg
