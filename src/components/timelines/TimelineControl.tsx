...
useEffect(() => {
	const ctx = gsap.context(() => {
		tl.current
			.from('.sun', { duration: 3, opacity: 0, x: 50, y: 50 })
			.from('.land', { duration: 1, opacity: 0 }, '-=1.5')
			.from('.gress', {
				duration: 1,
				opacity: 0,
				y: 100,
			...
			.from('.music', { duration: 1, opacity: 0, x: 100, y: 100 }, 3)
			.from('.eye-left,.eye-right', { duration: 1, x: 30, repeat: -1, yoyo: true })
			.to(
				'.mouse',
				{ scaleY: 0, transformOrigin: 'center center', repeat: -1, yoyo: true },
				'<',
			);
	}, tweenRef);
	return () => ctx.revert();
...
const TimelineControl = () => {
	...
	return (
		<PageLayout>
			<Typography variant='h1'>타임라인의 컨트롤과 라벨링</Typography>
			<Box ref={tweenRef} onClick={handleClick}>
				<Box sx={{ display: 'flex', gap: '1rem' }}>
					<Button variant='contained' className='play'>
						play
					</Button>
					<Button variant='contained' className='pause'>
						pause
					</Button>
					<Button variant='contained' className='reverse'>
						reverse
					</Button>
			...
			<Typography variant='body2'>
				{
					'GSAP은 타임라인에도 트윈과 동일하게 애니메이션을 제어할 수 있는 방법을 제공합니다\n\nLabel을 사용하면 타임라인에서 Label을 추가할 수 있습니다.\nadd() 메서드를 사용하여 타임라인에 Label을 추가할 수 있습니다.\n\n여기서는 test라는 Label을 추가하였습니다.\ntest 버튼을 누르면 "test" Label이 추가된 이후에 정의된 애니메이션이 play됩니다.'
				}
			</Typography>
			<CodeBlock
				language='tsx'
